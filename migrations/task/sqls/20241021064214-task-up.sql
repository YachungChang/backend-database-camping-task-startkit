...
WHERE user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND 
      course_id = (SELECT id FROM "COURSE" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io')) AND
      status = '即將授課';
...
FROM "COURSE_BOOKING"
WHERE "COURSE_BOOKING".user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') AND 
...
FROM(SELECT "CREDIT_PURCHASE".user_id, SUM("CREDIT_PURCHASE".purchased_credits) AS total 
     FROM "CREDIT_PURCHASE"
     WHERE "CREDIT_PURCHASE".user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io') 
     GROUP BY "CREDIT_PURCHASE".user_id
    ) AS "CREDIT_PURCHASE"
...
SELECT T3.name AS 教練名稱, T0.experience_years AS 經驗年數, T2.name AS 專長名稱
FROM "COACH" T0 INNER JOIN
     "COACH_LINK_SKILL" T1 ON T0.id = T1.coach_id INNER JOIN
     "SKILL" T2 ON T1.skill_id = T2.id INNER JOIN
     "USER" T3 ON T0.user_id = T3.id
WHERE T2.name = '重訓'
ORDER BY T0.experience_years DESC;

-- 6-2 查詢：查詢每種專長的教練數量，並只列出教練數量最多的專長（需使用 group by, inner join 與 order by 與 limit 語法）
-- 顯示須包含以下欄位： 專長名稱, coach_total

SELECT T2.name AS 專長名稱, COUNT(T3.email) AS 教練數量
FROM "COACH" T0 INNER JOIN
     "COACH_LINK_SKILL" T1 ON T0.id = T1.coach_id INNER JOIN
     "SKILL" T2 ON T1.skill_id = T2.id INNER JOIN
     "USER" T3 ON T0.user_id = T3.id
...
FROM "CREDIT_PACKAGE" INNER JOIN
     "CREDIT_PURCHASE" ON "CREDIT_PACKAGE".id = "CREDIT_PURCHASE".credit_package_id
WHERE date_part('month',"CREDIT_PURCHASE".purchase_at) = 12
...
FROM "CREDIT_PURCHASE"
WHERE date_part('month',"CREDIT_PURCHASE".purchase_at) = 12
GROUP BY date_part('month',"CREDIT_PURCHASE".purchase_at);
...
FROM "COURSE_BOOKING"
WHERE date_part('month',"COURSE_BOOKING".created_at) = 12 AND "COURSE_BOOKING".status <> '課程已取消';
